...
<template>
  <div class="rounded-xl border border-border bg-card p-8 shadow-sm">
    ...
    <form class="mt-6 space-y-4" @submit.prevent="submit">
      <div>
        <label class="mb-1.5 block text-sm font-medium" for="email">{{ t('login.email') }}</label>
        <input id="email" v-model="email" type="email" required autocomplete="email"
          class="w-full rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/40" placeholder="you@company.com" />
      </div>
      <div>
        ...
        <input id="password" v-model="password" type="password" required autocomplete="current-password"
          class="w-full rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/40" placeholder="••••••••" />
      </div>
    ...
    <p class="mt-6 text-center text-sm text-muted-foreground">
      {{ t('login.noAccount') }}
      <NuxtLink to="/signup" class="font-medium text-primary hover:underline">{{ t('login.createOne') }}</NuxtLink>
    </p>
  </div>
</template>
